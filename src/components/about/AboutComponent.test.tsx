...
describe("<AboutComponent />", () => {
  ...
  test("renders link to Odysee channel", () => {
    ...
    expect(link).toHaveAttribute("href", "https://odysee.com/@JulieBaronnieBeauty:7");
  ...
  test("renders warning about fake products", () => {
    render(<AboutComponent />);
    expect(
      screen.getByText(/foirfouille/i)
    ).toBeInTheDocument();
  });
  ...
  test("renders medical warning", () => {
    render(<AboutComponent />);
    expect(
      screen.getByText(/Seuls les médecins sont habilités à injecter sur autrui/i)
    ).toBeInTheDocument();
  });
  ...
  test("renders disclaimer about sharing practices", () => {
    render(<AboutComponent />);
    expect(
      screen.getByText(/Je partage ce que je fais et en aucun cas je ne vous incite/i)
    ).toBeInTheDocument();
  });
  ...
  test("renders warning about unlicensed practitioners", () => {
    render(<AboutComponent />);
    expect(
      screen.getByText(/n'allez jamais vous faire injecter chez des personnes/i)
    ).toBeInTheDocument();
  });
